...
{    
    ...
    {    	
 		$this->db->from('comments')->where(array('site_id' => $site_id, 'viewed' => 'N'));
 		return $this->db->count_all_results();
    }

    function add_comment($site_id, $user_id, $comment_data)
    {
 		$data = array(
 			'site_id'			=> $site_id,
		...
		return $this->db->get_where('comments', array('comment_id' => $status_id))->row();	
    }
